# Panopticon
[Panopticon AI](https://panopticon-ai.com/) 是一个基于Web的军事仿真平台，兼容 OpenAI Gym。我们的项目是开源的，
代码面向军事建模与仿真研究人员开放。
我们希望推动军事AI研究的技术发展。

平台：[我们的应用](https://app.panopticon-ai.com/)。

![image](https://github.com/user-attachments/assets/839cace4-dcbd-4abf-81b0-0b93afbfd8a8)

我们的 Discord 频道：[[https://discord.gg/gtUZAbbV](https://discord.gg/gtUZAbbV)](https://discord.gg/afHenKqu)

## 文档

详情请参阅[文档](https://docs.panopticon-ai.com/)，包括安装指南、操作教程、平台/游戏玩法信息等。

## 贡献

我们感谢并重视您的贡献！

在提交 Pull Request 之前，请务必阅读[贡献指南](https://github.com/Panopticon-AI-team/panopticon/blob/main/CONTRIBUTING.md)。

## 许可证

本项目采用 [Apache License 2.0](https://github.com/Panopticon-AI-team/panopticon/blob/main/LICENSE) 许可证。
