...
"""
实时遥测演示脚本
同时输出 TCP (Tacview) 和 WebSocket (前端) 数据流

启动后：
- Tacview 连接: 127.0.0.1:42674
- 前端连接: ws://127.0.0.1:8765
"""
...
# 配置流式传输（同时支持 TCP 和 WebSocket）
stream_config = StreamConfig(
    acmi_host="127.0.0.1",   # TCP 目标地址（Tacview）
    acmi_port=42674,          # Tacview 默认端口
    ws_host="127.0.0.1",     # WebSocket 地址（前端）
    ws_port=8765,             # WebSocket 端口
    send_interval=10,         # 每 10 步发送一次
)
...
if game.start_tcp_streaming():
    ...
    print(f"[Demo] + WebSocket 流式传输已启动: ws://{stream_config.ws_host}:{stream_config.ws_port}")
    print("[Demo] 请在 Tacview 中连接 TCP 地址")
    print("[Demo] 请在前端中连接 WebSocket 地址")
else:
    print("[Demo] X 流式传输启动失败")
